...
function updateMessage() {
    ...
    if (statusGame === Game.STATUS_LOSE) messageContainer.querySelector(".message-lose").classList.remove("hidden");
    else if (statusGame === Game.STATUS_WIN) messageContainer.querySelector(".message-win").classList.remove("hidden");
    else if (statusGame === Game.STATUS_IDLE) messageContainer.querySelector(".message-start").classList.remove("hidden");
}
...
class Game {
    ...
    moveLeft() {
        // [2, 2, 4, 0]
        if (this.status === Game.STATUS_WIN || this.status === Game.STATUS_LOSE || this.status === Game.STATUS_IDLE) return;
        ...
        for(let i = 0; i < this.state.length; i++){
            ...
            for(let j = 0; j < row.length - 1; j++)if (row[j] === row[j + 1]) {
                row[j] *= 2;
                row[j + 1] = 0; // [4, 0, 4]
                if (row[j] === Game.WIN_NUMBER) this.status = Game.STATUS_WIN;
                this.score += row[j];
            }
        ...
        if (!this.movesAvailable()) this.status = Game.STATUS_LOSE;
    }
    moveRight() {
        if (this.status === Game.STATUS_WIN || this.status === Game.STATUS_LOSE || this.status === Game.STATUS_IDLE) return;
        const prevState = JSON.stringify(this.state);
    ...
    moveUp() {
        ...
        for(let col = 0; col < this.state[0].length; col++){
            ...
            for(let i = 0; i < column.length - 1; i++)if (column[i] === column[i + 1]) {
                column[i] *= 2;
                column[i + 1] = 0;
                if (column[i] === Game.WIN_NUMBER) this.status = Game.STATUS_WIN;
                this.score += column[i];
            }
...
},{}]},["l1XHo","3cYfC"], "3cYfC", "parcelRequire79d6")
